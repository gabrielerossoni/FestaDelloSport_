...
# ================================
app = Flask(__name__)
app.secret_key = 'chiave-segreta-per-sessioni'  # CAMBIARE IN PRODUZIONE

# Configurazione OAuth Google
GOOGLE_CLIENT_ID = os.environ.get('GOOGLE_CLIENT_ID')
GOOGLE_CLIENT_SECRET = os.environ.get('GOOGLE_CLIENT_SECRET')
if not GOOGLE_CLIENT_ID or not GOOGLE_CLIENT_SECRET:
    print("[ERROR] GOOGLE_CLIENT_ID e GOOGLE_CLIENT_SECRET non trovati nelle variabili d'ambiente!")
    sys.exit(1)
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'admin_email' not in session:
            return jsonify({"error": "Accesso negato. Login admin richiesto."}), 401
...
# ================================
# OAUTH GOOGLE E DASHBOARD ADMIN
# ================================

@app.route('/login/google')
def login_google():
    """Inizia il processo di login OAuth con Google"""
...
def authorize_google():
    ...
    try:
        ...
        nome = user_info.get('name', 'Admin')
        
        # Verifica se l'email è nella lista admin
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT email FROM admin_emails WHERE email = %s", (email,))
        admin = cur.fetchone()
        cur.close()
        conn.close()
        
        if not admin:
            return jsonify({"error": "Email non autorizzata per accesso admin."}), 403
...
@app.route('/dashboard')
def dashboard():
    """Servi dashboard principale (richiede login)"""
    if 'admin_email' not in session:
...
def create_evento():
    ...
    try:
        ...
        print(f"Errore create evento: {e}")
        return jsonify({"error": "Errore interno"}), 500
...
def update_evento(evento_id):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            UPDATE eventi SET nome=%s, data=%s, luogo=%s, descrizione=%s, aggiornato_il=CURRENT_TIMESTAMP
            WHERE id=%s
        ...
        print(f"Errore update evento: {e}")
        return jsonify({"error": "Errore interno"}), 500
...
def delete_evento(evento_id):
    ...
    try:
        ...
        print(f"Errore delete evento: {e}")
        return jsonify({"error": "Errore interno"}), 500
...
def get_menu():
    ...
    try:
        ...
        print(f"Errore get menu: {e}")
        return jsonify({"error": "Errore interno"}), 500

@app.route('/api/admin/menu', methods=['POST'])
@admin_required
def create_menu_item():
    """Crea nuovo item menu"""
    try:
        data = request.get_json()
        nome = data['nome']
        descrizione = data.get('descrizione', '')
        ...
        print(f"Errore create menu item: {e}")
        return jsonify({"error": "Errore interno"}), 500
...
def update_menu_item(item_id):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            UPDATE menu_items SET nome=%s, descrizione=%s, prezzo=%s, categoria=%s, disponibile=%s, aggiornato_il=CURRENT_TIMESTAMP
            WHERE id=%s
        ...
        print(f"Errore update menu item: {e}")
        return jsonify({"error": "Errore interno"}), 500
...
def delete_menu_item(item_id):
    ...
    try:
        ...
        print(f"Errore delete menu item: {e}")
        return jsonify({"error": "Errore interno"}), 500
