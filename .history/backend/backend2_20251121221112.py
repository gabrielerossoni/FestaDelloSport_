...
The import statement `from flask import Flask, request, jsonify` and `from flask_cors import CORS` is needed because:

1. **Flask** - The web framework used to create the API server and define routes
2. **request** - To access incoming HTTP request data (GET/POST parameters, JSON body)
3. **jsonify** - To convert Python dictionaries into JSON responses
4. **CORS** - To enable Cross-Origin Resource Sharing, allowing frontend requests from different domains

These are essential dependencies for your backend API to function.
import os
import threading
