...
def init_database():
    ...
    print("[OK] Database PostgreSQL pronto")
...
def _serialize_rows(rows):
    """Converte liste di RealDictRow in liste di dict serializzando i timestamp"""
    serialized = []
    for row in rows:
        d = dict(row)
        for k, v in d.items():
...
def calcola_posti_occupati(data, ora, tavolo=None):
    ...
    try:
        ...
        if tavolo:
            ...
            occupati = {row["tavolo"]: row["ospiti_totali"] for row in results}
            cur.close()
            conn.close()
            return occupati
...
def get_prenotazioni():
    try:
        ...
        rows = cur.fetchall()
        prenotazioni = _serialize_rows(rows)
        cur.close()
        conn.close()
...
def get_feedback():
    try:
        ...
        rows = cur.fetchall()
        feedbacks = _serialize_rows(rows)
        cur.close()
        conn.close()
...
def post_reminder():
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO reminder_requests (contact) VALUES (%s) RETURNING id",
            (contact,)
        )
...
def get_reminder():
    try:
        ...
        rows = cur.fetchall()
        reminders = _serialize_rows(rows)
        cur.close()
        conn.close()
    ...
    try:
        ...
        stats = {
            ...
            "feedback": {
                ...
                "rating_medio": f"{float(stats_feedback['rating_medio']):.1f}" if stats_feedback["rating_medio"] else "0"
            },
            "promemoria": {
...
def start_server():
    try:
        ...
        print(f"[DB] Database PostgreSQL via env DATABASE_URL")
