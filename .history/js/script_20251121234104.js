...
function updateActiveNavLink() {
  ...
  for (const section of sections) {
    ...
    const sectionTop = rect.top + window.pageYOffset - navbarHeight - 8; // piccolo offset di tolleranza
  ...
  // fallback: se nessuna sezione è trovata, usa la prima disponibile
  if (!newActive && sections.length) newActive = sections[0].id;
...
// event listeners (aggiunti anche resize e load)
window.addEventListener("scroll", updateActiveNavLink, { passive: true });
window.addEventListener("resize", () => setTimeout(updateActiveNavLink, 100));
window.addEventListener("load", () => setTimeout(updateActiveNavLink, 50));
