...
function updateActiveNavLink() {
  ...
  // Scorri le sezioni e scegli quella visibile (usando bounding rect)
  for (const section of sections) {
    const rect = section.getBoundingClientRect();
    const sectionTop = rect.top + window.pageYOffset - navbarHeight - 8;
  ...
  // fallback: se siamo molto in alto, scegli la prima sezione
  if (!newActive) {
    if (scrollY < 120 && sections.length) {
      newActive = sections[0].id;
    } else {
      // se non trova nulla, prova ultima sezione visibile parzialmente
      for (let i = sections.length - 1; i >= 0; i--) {
        const s = sections[i];
        const rect = s.getBoundingClientRect();
        if (rect.top + window.pageYOffset - navbarHeight < scrollY + window.innerHeight * 0.5) {
          newActive = s.id;
          break;
        }
      }
  ...
  // Aggiorna classi sui link in modo sicuro (ignora link senza href valido)
  navLinks.forEach((link) => {
    link.classList.remove("active");
    const href = link.getAttribute("href");
    if (!href || !href.startsWith("#")) return;
    const id = href.slice(1);
    if (id === newActive) link.classList.add("active");
  });
}

// Aggiorna subito e sui principali eventi
window.addEventListener("scroll", updateActiveNavLink, { passive: true });
window.addEventListener("load", () => setTimeout(updateActiveNavLink, 60));
window.addEventListener("resize", () => {
  clearTimeout(resizeTimer);
  resizeTimer = setTimeout(updateActiveNavLink, 120);
});

// Quando si clicca un link: segna attivo subito (migliora UX) e aggiorna dopo scroll
navLinks.forEach((link) => {
  link.addEventListener("click", (e) => {
    const href = link.getAttribute("href");
    if (!href || !href.startsWith("#")) return;
    // rimuovi e aggiungi per evidenziare immediatamente
    navLinks.forEach(l => l.classList.remove("active"));
    link.classList.add("active");
    // lascia che lo smooth scroll finisca poi riallinea lo stato reale
