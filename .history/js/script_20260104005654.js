...
function formatPhoneError(phone) {
  ...
  if (!/^[\d\+\s\-\(\)]+$/.test(cleanPhone)) return "Il telefono contiene caratteri non validi";
  ...
  tableBtns.forEach((btn) => {
    ...
    reservationForm.addEventListener("submit", function (e) {
      ...
      if (!tableInput || !tableInput.value) {
        ...
        mostraErrore(errorMsg); // Usa la funzione esistente per l'animazione
        return;
      }
      ...
      if (errorMsg) {
        reservationError.textContent = errorMsg;
        reservationError.classList.remove("hidden");
        mostraErrore(errorMsg); // Animazione shake del bottone
        return;
      } else {
        reservationError.classList.add("hidden");
      }
  
      // Se tutto ok, procedi con l'invio (codice fetch rimane uguale)
      // ... resto del codice fetch rimane invariato ...
        ...
        fetchWithRetry(`${CONFIG.API_BASE_URL}/api/prenota`, {
          ...
          .then((res) => {
            ...
            if (!res.ok) {
              return res.json().then(data => {
                throw new Error(data.error || `Errore ${res.status}: ${res.statusText}`);
              }).catch(() => {
          ...
          .then((data) => {
            if (data.success) {
              ...
              reservationError.textContent = data.error || "Errore nella prenotazione.";
              reservationError.classList.remove("hidden");
            }
          })
          .catch((error) => {
            reservationError.textContent = getErrorMessage(error, "Errore di connessione al server. Riprova più tardi.");
            reservationError.classList.remove("hidden");
            mostraErrore(reservationError.textContent); // Animazione shake
          });
      });
  });
